...
async function getErrorLogById(rawId) {
  const id = parseBigIntId(rawId);

  const item = await prisma.errorLog.findUnique({
    where: { id },
    include: {
      admin: {
        select: {
  ...
  if (!item) {
    throw new AppError(404, 'NOT_FOUND', 'Error log not found');
  }

  return toDetail(item);
}

async function deleteAllErrorLogs() {
