...
const placementSchema = z.enum(['A', 'B', 'C', 'D', 'E']);
...
const createBannerSchema = z
  .object({
    ...
    translations: z.array(bannerTranslationSchema).min(1),
  })
...
const updateBannerSchema = z
  .object({
    ...
    fromDate: coerceNullableDate.optional(),
    toDate: coerceNullableDate.optional(),
    translations: z.array(bannerTranslationSchema).min(1).optional(),
  })
  .superRefine((data, ctx) => {
    const hasCore = data.placement !== undefined || data.fromDate !== undefined || data.toDate !== undefined;
...
const listBannersSchema = z.object({
  ...
  sortBy: z.enum(['id', 'placement', 'fromDate', 'toDate', 'createdAt', 'updatedAt']).optional().default('createdAt'),
